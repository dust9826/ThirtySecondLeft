Shader "BloodSystem/Blood_Tilemap"
{
    Properties
    {
        ...
        _WorldBoundsMax ("World Bounds Max", Vector) = (1, 1, 0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // 월드 좌표 → UV 변환
                float2 boundsSize = _WorldBoundsMax.xy - _WorldBoundsMin.xy;
                float2 bloodMaskUV = (input.positionWS.xy - _WorldBoundsMin.xy) / boundsSize;

                // 원본 타일맵 색상
                half4 mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uv);

                // 피 마스크 샘플링 (월드 좌표 기반 UV 사용)
                float bloodMask = SAMPLE_TEXTURE2D(_BloodMask, sampler_BloodMask, bloodMaskUV).r;
